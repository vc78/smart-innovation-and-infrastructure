...
import { Compass, CheckCircle2, XCircle, AlertTriangle, Brain, Sparkles, Zap, RotateCcw } from "lucide-react"
...
export function VastuChecker() {
  ...
  const handleAutoAlignIdeal = () => {
    ...
    toast.success("Aligned all rooms to ideal Vedic Vastu Purusha directions!")
  }
    ...
    <div className="space-y-8">
      <Card className="p-4 sm:p-6 md:p-8 border-border/50 shadow-2xl rounded-2xl sm:rounded-[2rem] bg-background/50 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 md:mb-8 gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
             <Button
               variant="outline"
               size="sm"
               onClick={handleAutoAlignIdeal}
               className="text-xs font-bold gap-1.5 border-amber-500/30 text-amber-600 dark:text-amber-400 hover:bg-amber-500/10"
             >
               <Zap className="w-3.5 h-3.5" /> Auto-Align Ideal
             </Button>
             <Badge variant="outline" className="px-2 sm:px-3 py-1 font-bold text-[10px] sm:text-xs whitespace-nowrap">
               GRADE: {evaluation.grade}
             </Badge>
             <Badge
               className={`px-3 sm:px-4 py-1 text-[10px] sm:text-xs font-bold border-transparent shadow-lg flex-1 sm:flex-none text-center ${evaluation.overall_score >= 75 ? 'bg-emerald-600 text-white' : evaluation.overall_score >= 55 ? 'bg-amber-500 text-white' : 'bg-red-500 text-white'}`}
             >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="p-3 sm:p-4 rounded-2xl bg-muted/30 border border-border flex items-center gap-4">
            <Switch id="gemini-mode-checker" checked={useGeminiMode} onCheckedChange={setUseGeminiMode} />
            <Label htmlFor="gemini-mode-checker" className="cursor-pointer">
              <span className="block text-xs sm:text-sm font-bold uppercase">Gemini AI Live Engine</span>
              <span className="text-[9px] sm:text-[10px] text-muted-foreground font-medium">Real-time solar & elemental alignment</span>
            </Label>
          </div>
          <div className="p-3 sm:p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20 flex items-center justify-around">
        ...
        <div className="space-y-4 max-h-[460px] overflow-y-auto pr-2 custom-scrollbar">
          {VASTU_ELEMENTS.map((el) => {
            ...
            const ruleRes = evaluation.defects.find(d => d.rule.includes(el.key)) || { severity: "ok", defect: "Perfectly Aligned with Vedic Principles", recommendation: "" }
            
            return (
              <div
                key={el.key}
                className={`p-4 rounded-2xl border transition-all duration-300 ${getStatusClass(ruleRes.severity)} shadow-sm hover:shadow-md`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    {getStatusIcon(ruleRes.severity)}
                    <span className="font-bold text-xs sm:text-sm uppercase tracking-tight">{el.element}</span>
                  </div>
                  <select
                    value={currentDir}
                    onChange={(e) =>
                    ...
                    className="text-xs font-semibold border rounded-xl px-3 py-1.5 bg-background shadow-sm focus:ring-2 ring-amber-500/20 outline-none"
                  >
                    {DIRECTIONS.map((dir) => (
                ...
                <div className="space-y-1.5">
                  <p className="text-xs font-bold text-foreground leading-snug">
                    {ruleRes.severity === "ok" ? "Auspicious directional placement verified." : ruleRes.defect}
                  </p>
                  {ruleRes.recommendation && (
                    <div className="p-2.5 rounded-xl bg-white/60 dark:bg-black/30 border border-border/50 flex items-start gap-2">
                       <Sparkles className="w-3.5 h-3.5 text-amber-500 mt-0.5 flex-shrink-0" />
                       <p className="text-[10px] font-medium text-muted-foreground leading-tight">Remedy: {ruleRes.recommendation}</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* Full Gemini Vastu Layout & Vector CAD Blueprint Generator */}
