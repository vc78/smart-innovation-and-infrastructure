...
export async function POST(req: Request) {
  try {
    ...
    // Impact of Rooms (Arbitrary rule from old code)
    ...
    amenities.forEach((id: string) => {
      ...
      if (amenityData && tier in amenityData) {
        ...
        if (typeof val === 'number') {
           ...
           // use city cost_per_sqft (economy tier) x 120 sqft
    ...
    if (kitchenData && typeof kitchenData === 'object' && 'avg' in kitchenData) {
       // not provided as avg, so we just calculate midpoint
    ...
    const cementPrice = MATERIAL_DATASET.materials.cement_per_50kg_bag_inr[cement as keyof typeof MATERIAL_DATASET.materials.cement_per_50kg_bag_inr]?.avg || 375
    const steelPrice = MATERIAL_DATASET.materials.tmt_steel_per_kg_inr[steel as keyof typeof MATERIAL_DATASET.materials.tmt_steel_per_kg_inr]?.avg || 60
